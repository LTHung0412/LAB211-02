...
public class Flight {
    ...
    public int findPassenger(Passenger passenger, Reservation reservation) {
        int index = -1;
        for (int i = 0; i < passengerSeats.length; i++) {
            if (reservation.getPassenger().getName().equals(passenger.getName()) && reservation.getFlight().getNumber().equals(this.number)) {
                index = i;
                break;
            }
        }
